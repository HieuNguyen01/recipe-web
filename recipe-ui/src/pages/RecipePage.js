...
import {
  Container, Grid, Card, CardMedia, CardContent, CardActions, Icon, Divider, Box,
  Dialog, DialogContent, DialogActions, TextField, Link, Typography, Breadcrumbs, CircularProgress
} from "@mui/material";
...
export default function RecipePage() {
  ...
  async function handleSave() {
    ...
    try {
      ...
      console.error(err);
      enqueueSnackbar(
        err.response?.data?.message || "Failed to edit recipe",
        { variant: "error" }
      );
    } finally {
      setIsSaving(false);
  ...
  const handleDelete = () => {
    openConfirm({
      ...
      contentText: 'Are you sure you want to permanently delete this recipe?',
      confirmText: 'Delete',
      cancelText: 'Cancel',
      confirmButtonProps: {
        variant: 'outlined',
    ...
    <MKBox component="main" py={6}>
      <Container maxWidth="md">
        ...
        <Dialog open={commentDialog} onClose={closeComment}>
          ...
          <DialogActions>
            <MKButton onClick={submitComment} color="success">Submit</MKButton>
            <MKButton onClick={closeComment} variant="outlined" color="secondary">Cancel</MKButton>
          </DialogActions>
        </Dialog>
      </Container>
      {/* ─── Confirm Dialog ─── */}
